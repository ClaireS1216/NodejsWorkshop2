...
partnerRouter
    ...
    Partner.findById(req.params.partnerId)
      .then((partner) => {
        if (partner && partner.comments.id(req.params.commentId)) {
          ...
          err = new Error(`Partner ${req.params.parnterId} not found`);
          err.status = 404;
          return next(err);
        } else {
          err = new Error(`Comment ${req.params.commentId} not found`);
  ...
  .delete((req, res, next) => {
    Partner.findById(req.params.partnerId)
      .then((partner) => {
        if (partner && parnter.comments.id(req.params.commentId)) {
          ...
          err = new Error(`Parnter ${req.params.partnerId} not found`);
          err.status = 404;
          return next(err);
        } else {
          err = new Error(`Comment ${req.params.commentId} not found`);
